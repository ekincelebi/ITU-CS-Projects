...
def loop_in_images_video_background(images, backgroundList, listToAppend):
    for i in range(len(images)):
        ...
        foreground = np.logical_or(image_g_channel < 180, image_r_channel > 150)  # okeyyyyy
        nonzero_x, nonzero_y = np.nonzero(foreground)
...
def add_noise_to_histogram(histogram):
    # adds noise to given histogram

    array = np.array(histogram)
    shape = array.shape
    noise = np.random.normal(0, .1, shape)
    new_signal = array + noise * 1500
    return new_signal
...
disco_cat = []
#make histogram matching for per cat frame for the left cat

for cat in cat_images:
    #calculate each cats histogram
    catHistogram = foreground_histogram(cat)
    noisyHistogram = add_noise_to_histogram(catHistogram)  #add random noise to the histogram
    disco = histogram_matching(catHistogram,noisyHistogram,cat)
    #disco = disco[:, :, [2, 1, 0]]
    disco_cat.append(disco)
...
for flip in flipped_cats:
    #calculate each cats histogram
    flipHistogram = foreground_histogram(flip)
    noisyFlip = add_noise_to_histogram(flipHistogram) #add random noise to the histogram
    discoRight = histogram_matching(noisyFlip,flipHistogram,flip)
    #disco = disco[:, :, [2, 1, 0]]
    disco_cat_right.append(discoRight)


disco_left = []
loop_in_images(disco_cat,background,disco_left)
...
clip.write_videofile('lastPart.mp4', codec='libx264')
